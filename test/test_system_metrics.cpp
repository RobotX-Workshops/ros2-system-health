...
static float test_memory_usage_percentage()
{
    ...
    if (!meminfo.is_open()) return -1.0f;
...
TEST(SystemMetrics, MemoryUsageNonZero)
{
    ...
    const float mem = test_memory_usage_percentage();
    if (mem < 0.0f) GTEST_SKIP() << "/proc/meminfo not available";
...
{
    const float disk = test_disk_usage_percentage();
    if (disk < 0.0f) GTEST_SKIP() << "statvfs not available";
...
TEST(SystemMetrics, IpAddressEmptyInterfaceReturnsNA)
{
    EXPECT_EQ(test_ip_address(""), "N/A");
}
